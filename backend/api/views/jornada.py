from rest_framework import status
from rest_framework.response import Response
from django.http.response import Http404
from rest_framework.views import APIView

from api.models import Jornada
from api.serializers import JornadaSerializer


class JornadaList(APIView):
    queryset = Jornada.objects.all()
    serializer_class = JornadaSerializer

    def get(self, request):
        jornadas = Jornada.objects.filter(creador=request.user)
        serializer = JornadaSerializer(jornadas, many=True)
        return Response(serializer.data)

    def post(self, request):
        serializer = JornadaSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save(creador=request.user)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


class JornadaDetail(APIView):
    queryset = Jornada.objects.all()
    serializer_class = JornadaSerializer

    def get_object(self, pk):
        try:
            return Jornada.objects.get(pk=pk)
        except Jornada.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        jornada = self.get_object(pk)
        serializer = JornadaSerializer(jornada)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        jornada = self.get_object(pk)
        serializer = JornadaSerializer(jornada, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def delete(self, request, pk, format=None):
        jornada = self.get_object(pk)
        jornada.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)